...
loc_feat = A / "per_image_features_hcg_h_gate025_risk_inv_detach_s044_min090_local_delta_cap080_rho1_seed1234_step250_val4096_holdout4096_current.csv"
...
methods = {
    ...
    "local cap080 rho1": method_stats("local cap080 rho1", "local_rd"),
}
for name in ["old gate0.25", "trained min090 risk", "local cap080 rho1"]:
...
features = {
    ...
    "local cap080 rho1": feature_summary(loc_feat),
}
...
summary = {
    ...
    "artifacts": {
        ...
        "feature_distribution": "experiments/analysis/feature_distribution_hcg_h_gate025_risk_inv_detach_s044_min090_local_delta_cap080_rho1_seed1234_step250_val4096_holdout4096_current.json",
        "per_image_features": str(loc_feat),
    },
    "num_images": len(joined),
}
json_path = A / "local_delta_cap080_rho1_seed1234_val4096_holdout4096_current.json"
json_path.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")

lines = [
    "# Local Delta Cap Probe Audit",
    "",
    "Seed1234 holdout4096 audit for `rho_householder_delta_local_cap=1.0` and `householder_delta_local_cap=0.080` on the min090 inverse/detached risk gate.",
    "",
    "## Overall RD",
    "",
...
lines += [
    ...
    "## Interpretation",
    "",
    "- The local cap is rejected as configured. It does not selectively trim harmful local geometry; it globally under-powers Householder geometry.",
    "- The best checkpoint has much higher RD than HCS/old/min090 despite scale returning near the old operating point. The geometry strength and delta RMS are below the validated old gate0.25 regime, while per-image dead-code ratio rises sharply.",
    "- This supports the earlier E050/E051 diagnosis: preserving scale or hitting a simple displacement bound is not enough. The next controller should use a geometry floor/trust region or softer staged penalty, and it must explicitly preserve decoder-compatible transform strength/codebook use.",
]
md_path = A / "local_delta_cap080_rho1_seed1234_val4096_holdout4096_current.md"
md_path.write_text("\n".join(lines) + "\n")
print(md_path)
print(json_path)
print("local RD", methods["local cap080 rho1"]['rd'])
print("local-HCS", methods["local cap080 rho1"]['delta_vs_hcs'])
