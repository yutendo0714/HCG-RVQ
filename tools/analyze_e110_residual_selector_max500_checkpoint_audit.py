#!/usr/bin/env python3
"""Audit residual-selector checkpoint budget: step250 versus max500."""
...
OUT_PREFIX = ANALYSIS / "e110_residual_selector_max500_checkpoint_audit"
...
def pairwise_checkpoint_rows(
    ...
    for threshold in THRESHOLDS:
        for seed in SEEDS:
            ...
            out.append(row)
            aggregate[threshold].extend({"delta": d} for d in diffs)

        all_diffs = [item["delta"] for item in aggregate[threshold]]
        out.append(
            {
                "threshold": threshold,
                "seed": "all",
...
def main() -> None:
    ...
    decision = {
        ...
        "interpretation": (
            "max500 improves the 3-seed mean relative to step250, but seed3456 becomes worse "
            "than its beta reference. This is a useful high-mean candidate, not yet a clean "
            "paper-main checkpoint rule unless selected by an independent protocol."
        ),
    }
    ...
    lines = [
        ...
        "",
        (
            f"- Best 3-seed mean: {decision['best_mean_budget']} dz{decision['best_mean_threshold']} "
            f"with delta {fmt(decision['best_mean_delta'])}."
        ),
        (
        ...
        (
            f"- Seed3456 max500 remains fragile: dz014 {fmt(decision['seed3456_max500_dz014_delta'])}, "
            f"dz018 {fmt(decision['seed3456_max500_dz018_delta'])}."
        ),
    ...
    lines.extend(
        [
            ...
            "## Notes",
            "",
            "- All E110 runs were executed with CUDA_VISIBLE_DEVICES=0 and cuda:0.",
            "- nonfinite_rows is 0 for every max500 seed and threshold.",
            "- The high-mean max500 result should be treated as a checkpoint-budget candidate; a paper-main claim still needs either an independent checkpoint selection rule or a stabilization experiment that removes the seed3456 regression.",
        ]
    )
