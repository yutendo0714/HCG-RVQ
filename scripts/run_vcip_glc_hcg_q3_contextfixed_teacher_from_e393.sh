...
# E431 / VCIP focused triage.
# Context-preserving HCG-RVQ: keep original scalar GLC quantized parts as
# autoregressive context, and apply HCG-RVQ only to the final reconstructed
# latent. This tests whether hard-branch failures were caused by corrupting
# GLC spatial-prior context rather than by HCG-RVQ itself.
...
OUT_ROOT="${OUT_ROOT:-/workspace/HCG-RVQ/experiments/analysis/e431_glc_hcg_q3_contextfixed_teacher_from_e393_$(date +%Y%m%d_%H%M%S)}"
...
# Keep the objective close to the HCG story: protect GLC spatial context and train a usable final-latent HCG correction.
# This is intentionally not an FID/KID-specific loss.
BRANCH_IMAGE_WEIGHT="${BRANCH_IMAGE_WEIGHT:-0.50}"
...
TEACHER_SOFT_WEIGHT="${TEACHER_SOFT_WEIGHT:-0.50}"
...
for seed in $SEEDS; do
  ...
  prefix="${OUT_ROOT}/glc_hcg_q3_contextfixed_teacher_seed${seed}_steps${STEPS}_batch${TRAIN_BATCH_PER_STEP}"
  wandb_args=()
  if [[ "$WANDB_ENABLED" != "0" ]]; then
    wandb_args+=(--wandb-enabled --wandb-project "$WANDB_PROJECT" --wandb-mode "$WANDB_MODE")
    wandb_args+=(--wandb-name "e431_q3_contextfixed_teacher_seed${seed}_steps${STEPS}")
