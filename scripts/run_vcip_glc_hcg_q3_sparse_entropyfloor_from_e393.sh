...
# E427 / VCIP focused triage.
# Recover the strong E393 perceptual headroom under deployable accounting.
# Key changes after E425:
# - q3 only: reduce mixed-q interference and focus on the most promising strict-accounting regime.
# - sparse E393-compatible parts/groups only: avoid the all-groups codebook collapse observed in E425.
# - soft-index entropy floor: prevent low-entropy/dead-code collapse while keeping the high-entropy cap.
# - hard/ST gate + rate pressure: optimize the deployable hard path, not only soft analysis rows.
...
OUT_ROOT="${OUT_ROOT:-/workspace/HCG-RVQ/experiments/analysis/e427_glc_hcg_q3_sparse_entropyfloor_from_e393_$(date +%Y%m%d_%H%M%S)}"
...
# Simple latent/residual-quality objective, not metric-specific overfitting.
BRANCH_IMAGE_WEIGHT="${BRANCH_IMAGE_WEIGHT:-1.0}"
LPIPS_WEIGHT="${LPIPS_WEIGHT:-0.35}"
DISTS_WEIGHT="${DISTS_WEIGHT:-1.0}"
...
for seed in $SEEDS; do
  ...
  prefix="${OUT_ROOT}/glc_hcg_q3_sparse_entropyfloor_seed${seed}_steps${STEPS}_batch${TRAIN_BATCH_PER_STEP}"
  wandb_args=()
  if [[ "$WANDB_ENABLED" != "0" ]]; then
    wandb_args+=(--wandb-enabled --wandb-project "$WANDB_PROJECT" --wandb-mode "$WANDB_MODE")
    wandb_args+=(--wandb-name "e427_q3_sparse_entropyfloor_seed${seed}_steps${STEPS}")
...
echo "[next eval CLIC250 exact/deployable example]"
echo "CUDA_VISIBLE_DEVICES=0 INPUT_PATH=/dpl/clic/professional/test ACTIVE_THRESHOLD=${ACTIVE_THRESHOLD} FID_PATCH_SIZE=-1 Q_INDEXES=\"3\" EXPORT_LABELS=\"base replacement_hard replacement_soft\" REPLACEMENT_SIGNAL_BITS=\"1 8\" LEARNED_HARD_MIN_Q=\"3\" bash scripts/eval_vcip_glc_hcg_deployable_clic64.sh CHECKPOINT.pt OUT_DIR"
